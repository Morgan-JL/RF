...
import com.czx.framework.utils.CharUtil;
import com.czx.web.servlet.Result;
import com.czx.web.utils.FileUtil;
import com.czx.web.route.BeanMapping;
import com.czx.web.route.RouteContainer;
import com.czx.web.servlet.Model;
import com.czx.web.servlet.param.ParameterAnalysisChain;
  ...
  public static void execute(HttpServletRequest req, HttpServletResponse resp) {
    ...
    try {
      ...
      if ("/".equals(model.getRequest().getServletPath())) {
        ...
        //获取路由容器
        RouteContainer routeContainer = RouteContainer.create();
        //获取路由映射对象
        BeanMapping mapping = routeContainer.getBeanMapping(req.getServletPath());
        //路由参数解析
